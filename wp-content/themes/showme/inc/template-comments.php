            ...
            <div class="comment_wrap">
              <div class="fn"><?php echo get_comment_author_link();?></div>
                ...
                <div class="comment-meta clearfix">
                  ...
                  <?php edit_comment_link( esc_html__( 'Edit', 'showme' ), '<span class="comment-edit"><i class="fa fa-edit" aria-hidden="true"></i> ', '</span>' ); ?>
                </div>
...
function showme_comment_form_default_fields( $fields ) {
  ...
  $fields['author'] = '<div class="comment-item"><label for="author" class="screen-reader-text">Name</label><input id="author" type="text" aria-required="true" size="22" value="'.esc_attr($commenter['comment_author']).'" name="author" '.$aria_req.' placeholder="'.esc_attr__('Your Name','showme').' '.($req?'*':'').'" /></div>';
  $fields['email'] = '<div class="comment-item"><label for="email" class="screen-reader-text">Email</label><input id="email" type="text" aria-required="true" size="22" value="'.esc_attr($commenter['comment_author_email']).'" name="email" '.$aria_req.' placeholder="'.esc_attr__('Your Email','showme').' '.($req?'*':'').'" /></div>';
  $fields['url'] = '<div class="comment-item"><label for="url" class="screen-reader-text">Website</label><input id="url" type="text" aria-required="true" size="22" value="'.esc_url($commenter['comment_author_url']).'" name="url" placeholder="'.esc_attr__('Your Website','showme').'" /></div>';
  return $fields;
}
...
function showme_comment_form_field_comment( $comment_field ) {
  $req = get_option( 'require_name_email' );
  $comment_field = '<div class="comment-item"><label for="comment" class="screen-reader-text">Comment</label><textarea id="comment" name="comment" placeholder="'.esc_attr__('Your comment','showme').' '.($req?'*':'').'" /></textarea></div>';
